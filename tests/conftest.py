...
``temp_db`` gives a test the real dev database (TEST_DB_* in .env) with
...
def _dev_connection():
    ...
    if not host or host == os.getenv("DB_HOST"):
        return None  # never run against prod
    try:
        return psycopg2.connect(
            user=os.getenv("TEST_DB_LOGIN"),
            password=os.getenv("TEST_DB_PASS"),
            host=host,
            port=int(os.getenv("TEST_DB_PORT") or 5432),
            database=os.getenv("TEST_DB_DATABASE"),
            sslmode=os.getenv("TEST_DB_SSLMODE") or None,
            connect_timeout=5,
        )
    except Exception:
        return None
...
def _dev_db():
    ...
    if conn is None:
        pytest.skip("dev database unreachable (TEST_DB_* in .env)")
